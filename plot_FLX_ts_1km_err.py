...
def read_FLUXNET_site(start_date,end_date,location,base_dir,var_flx):
   ...
   # Example: Find the file paths for all locations
    ...
    df_FLX_site['TIMESTAMP_START'] = df_FLX_site['TIMESTAMP_START'] - pd.Timedelta(hours=1) # TODO: make sure time change is handled correcly
...
csv_files_sorted = sorted(
    csv_files,
    key=lambda x: int(x.split(f'_sites_')[1].split('km')[0]),  # Extract distance as an integer
...
# locations = [ "IT-Ren","CH-Dav", "IT-Lav", "AT-Neu", "IT-MBo"] 
locations = ["CH-Dav"] 
...
pft_site_match = pd.read_csv(f'/home/c707/c7071034/Github/WRF_VPRM_post/pft_site_match_at_{res_dx}.csv')
model_lat_lon = []
...
for location in locations:
    for fluxtype,unit in zip(fluxtypes,units):
        ...
        # variables = {f"ref_{fluxtype}": "solid" ,f"std_{fluxtype}": "dashdot" , f"tune_{fluxtype}": "dashed" , "std_GPP_Pmodel" : "dotted", "std_RECO_Migli" : "dotted", "std_NEE_PMigli" : "dotted"} 
        variables = {f"REF_{fluxtype}": "solid" ,f"ALPS_{fluxtype}": "dashdot" , f"SITE_{fluxtype}": "dashed" , "ALPS_GPP_Pmodel" : "dotted", "ALPS_RECO_Migli" : "dotted", "ALPS_NEE_PMigli" : "dotted"} 
        ...
        for resolution, df in dataframes.items():
            ...
            if fluxtype == "NEE_WRF":
                ...
                nee_columns = {} # Dynamically create NEE columns by summing corresponding GPP and RECO columns
                nee_columns_P = {}
                for col in df_loc.columns:
                    if '_GPP_WRF' in col:
                        prefix = col.replace('_GPP_WRF', '')
                        if f'{prefix}_RECO_WRF' in df_loc.columns:
                            nee_columns[f'{prefix}_NEE_WRF'] = (f'{prefix}_RECO_WRF', f'{prefix}_GPP_WRF')
                for col in df_loc.columns:
                    if '_GPP_Pmodel' in col:
                        prefix = col.replace('_GPP_Pmodel', '')
                        if f'{prefix}_RECO_Migli' in df_loc.columns:
                            nee_columns_P[f'{prefix}_NEE_PMigli'] = (f'{prefix}_RECO_Migli', f'{prefix}_GPP_Pmodel')

                for nee_col, (res_col, gee_col) in nee_columns.items():
                    df_loc[nee_col] = df_loc[res_col] + df_loc[gee_col]
                df_loc = df_loc.drop(columns=df_loc.filter(regex="(_GPP_WRF|_RECO_WRF)$").columns)
                for nee_col_P, (reco_col, gpp_col) in nee_columns_P.items():
                    df_loc[nee_col_P] = df_loc[reco_col] - df_loc[gpp_col]
                df_loc = df_loc.drop(columns=df_loc.filter(regex="(_GPP_Pmodel|_RECO_Migli)$").columns)
            elif fluxtype == "T2_WRF":
                df_loc = df.filter(regex=f"^{location}_REF(.*_{fluxtype})$")  # Match location and variable pattern
                df_loc = df_loc - 273.15
            elif fluxtype == "GPP_WRF":
                df_loc = -df.filter(regex=f"^{location}_(.*_{fluxtype})$")  # turning GPP to positive values
            ...
            site_match_all = pd.read_csv(f"pft_site_match_at_{resolution}.csv")
            site_match = site_match_all[site_match_all["site"] == location]
            df_FLX_site_copy = df_FLX_site.copy()

            for column in df_loc.columns:
                variable = column.split("_")[-3] +"_"+ column.split("_")[-2] +"_"+ column.split("_")[-1]  # Extract the variable type (e.g., ref_GPP_WRF, std_GPP_WRF)
                if variable in variables:
                    ...
                    if "TIMESTAMP_START" in df_FLX_site_copy.columns:
                        df_FLX_site_copy["TIMESTAMP_START"] = pd.to_datetime(df_FLX_site_copy["TIMESTAMP_START"]).dt.tz_localize(None)
                        df_FLX_site_copy = df_FLX_site_copy.set_index("TIMESTAMP_START")

                    # Now filter by df_loc index
                    df_FLX_site_copy = df_FLX_site_copy[df_FLX_site_copy.index.isin(df_loc.index)]

                    # Align index/order exactly
                    df_FLX_site_copy = df_FLX_site_copy.loc[df_loc.index]

                    hourly_avg = df_loc.groupby("hour")[column].mean()
                            # mask out nan values from both time series
                    # filter values in df_FLX_site_copy based on the dates of df_loc
                    if df_loc[column].isna().any():
                        mask = ~df_loc[column].isna()
                        clean_values_FLX = df_FLX_site_copy[var_flx][mask.tolist()].tolist()
                        ...
                        evaluator = RegressionMetric(
                            df_FLX_site_copy[var_flx].tolist(),
                            df_loc[column].tolist(),
                        )
                    metrics = evaluator.get_metrics_by_list_names(["RMSE","MSE", "MAE", "MAPE", "R2", "NNSE", "KGE"]) 
                    ...
                    if variable == "std_GPP_Pmodel" or variable == "std_RECO_Migli" or variable == "std_NEE_PMigli":
                        color_i = "purple"                        
                    ...
                    plt.plot(
                        ...
                        label = (f"{resolution}_{variable} RMSE={metrics['RMSE']:.2f} R2={metrics['R2']:.2f}"),
                        linestyle=linestyle,
                        color=color_i,
                    )

        if plot_CAMS:
            df_FLX_site_copy2 = df_FLX_site.copy()
            if location in df_CAMS_hourly_all:
                df_CAMS_hourly = df_CAMS_hourly_all[location]
            df_CAMS_hourly["hour"] = df_CAMS_hourly.index.hour
            # merge df_FLX_site and df_CAMS_hourly

            # # Merge df_FLX_site and df_CAMS_hourly on their timestamps
            # df_FLX_site.index = pd.to_datetime(df_FLX_site.index)
            df_FLX_site_copy2.set_index('TIMESTAMP_START', inplace=True)
            #TODO: These is some bug here, fix it!

            # # Ensure both DataFrames have the same timezone awareness
            df_FLX_site_copy2.index = df_FLX_site_copy2.index.tz_localize(None)
            df_CAMS_hourly.index = df_CAMS_hourly.index.tz_localize(None)

            # Merge df_FLX_site and df_CAMS_hourly on their timestamps
            df_merged = pd.merge(df_FLX_site_copy2, df_CAMS_hourly, left_index=True, right_index=True, suffixes=('_FLX', '_CAMS'))
            cols_to_fix = ['fco2gpp', 'fco2gpp_bfas', 'fco2rec', 'fco2rec_bfas', 'fco2nee', 'fco2nee_bfas', 't2m']
            df_merged[cols_to_fix] = df_merged[cols_to_fix].apply(pd.to_numeric, errors='coerce')
            df_merged[cols_to_fix] = df_merged[cols_to_fix].interpolate(method='linear')
            cams_l = df_merged[var_CAMS_plot]

            evaluator = RegressionMetric(
                df_merged[var_flx].tolist(),
                cams_l.tolist(),
                )
            metrics = evaluator.get_metrics_by_list_names(["RMSE","MSE", "MAE", "MAPE", "R2", "NNSE", "KGE"]) 
            # save metric data for csv
            for metric, value in metrics.items():
                consolidated_metrics_df.loc[metric, var_CAMS_plot+"_"+resolution] = value  # Assign metric value to the appropriate column
            ...
            if var_CAMS_plot == "t2m":
                ...
                plt.plot(
                        hourly_avg_CAMS,  
                        label=f"CAMS {var_CAMS_plot} RMSE={metrics['RMSE']:.2f}  R2={metrics['R2']:.2f} ",
                        color="orange",
                )
            else:
                hourly_avg_CAMS = df_CAMS_hourly#.interpolate(method='linear')
                ...
                plt.plot(
                        ...
                        label=f"CAMS {var_CAMS_plot}_bfas corr. MSE={metrics['MSE']:.2f}  R2={metrics['R2']:.2f} ",
                        color="orange",
                )

        df_FLX_site_copy2["hour"] = df_FLX_site_copy2.index.hour
        hourly_avg_FLX = df_FLX_site_copy2.groupby("hour")[var_flx].mean()
        plt.plot(
                hourly_avg_FLX, 
        ...
        # plt.title(f"{var_flx}, CAMS and WRF {method} at {location} - {locations_hgt[location]}m ASL \n at {site_match['pft'].values[0]}, dist. to site {site_match['dist_km'].values[0]:4.1f}km, ") 
        plt.xlabel("UTC [h]", fontsize=14*factor_fontsize)
        if unit == "[°C]":
            plt.ylabel("[°C]", fontsize=14*factor_fontsize)
        else:
            plt.ylabel("[μmol/m²/s]", fontsize=14*factor_fontsize)
        plt.tick_params(labelsize=12*factor_fontsize)
        plt.legend(loc='center', bbox_to_anchor=(0.5, -0.25), ncol=2, fontsize=8*factor_fontsize)
        plt.xticks([0, 6, 12, 18, 24]) 
        plt.grid()
        plt.tight_layout()
        plt.savefig(f"{outfolder}/{location}_{fluxtype}_comparison_hourly_means_{timespan}.pdf", bbox_inches="tight")  # Save plot as PNG
        plt.show()
