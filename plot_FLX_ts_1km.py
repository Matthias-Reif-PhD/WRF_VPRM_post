...
for location in locations:
    for fluxtype,unit in zip(fluxtypes,units):
        ...
        variables = {f"SITE_{fluxtype}": "solid",f"ALPS_{fluxtype}": "dashdot"  , f"REF_{fluxtype}": "dashed"} 
        # for Pmodel:
            ...
            for column in df_loc.columns:
                ...
                if variable in variables:
                    ...
                    if "TIMESTAMP_START" in df_FLX_site.columns:
                        df_FLX_site["TIMESTAMP_START"] = pd.to_datetime(df_FLX_site["TIMESTAMP_START"]).dt.tz_localize(None)
        ...
        if plot_CAMS:
            ...
            df_merged = pd.merge(df_FLX_site, df_CAMS_hourly, left_index=True, right_index=True, suffixes=('_FLX', '_CAMS'))
        ...
        # plt.show()
        print(f"finished: {location}_{fluxtype}_comparison_hourly_means_{timespan}")
