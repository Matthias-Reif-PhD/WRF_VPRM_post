...
for coarse_domain in coarse_domains:
    ...
    for STD_TOPO in STD_TOPOs:
        for STD_TOPO_flag in STD_TOPO_flags:
            ...
            print(
                f"Mean difference in height between {coarse_domain} and 3km is {diff_hgt_mean:.2f} m"
            )
            print(
                f"Mean of 3km is {np.nanmean(hgt_3km):.2f} and of {coarse_domain} is {np.nanmean(hgt_coarse_domain):.2f} with difference  {diff_hgt_mean_nonproj:.2f}  "
            )
