...
for m in range(7):
    vegfrac = veg_frac_map[0, m, :, :].values
    ...
    if PAR0 > 0:
        ...
        RADscale_1km += RADscale_temp * vegfrac
    ...
    Tscale_tmp[np.isnan(Tscale_tmp)] = 0  # set nan values of Tscale_1km to zeros
    ...
    Wscale_temp_1km[np.isnan(Wscale_temp_1km)] = 0 #  set nan values of Wscale_1km to zeros
    Wscale_1km += Wscale_temp_1km
    

    # --- Pscale ---
    if m == 0:  # evergreen
        Pscale_temp_1km = 1.0 * vegfrac
    ...
    Pscale_temp_1km[np.isnan(Pscale_temp_1km)] = 0
    ...
    if PAR0 > 0:
        ...
        RADscale_d01 += RADscale_temp_d01 * vegfrac
    ...
    Tscale_d01 += np.where(Tscale_tmp_d01 < 0, 0, Tscale_tmp_d01) * vegfrac
    ...
    Wscale_d01 += Wscale_temp_d01
    
    # --- Pscale ---
    if m == 0:  # evergreen
        Pscale_temp_d01 = 1.0 * vegfrac
    elif m == 4 or m == 6:  # savanna / grassland
    ...
    Pscale_d01 += Pscale_temp_d01 * vegfrac
    ...
    GPP_test_d01 += GPP_temp_d01
    Residual_d01 += (lambda_of_PFT[list(lambda_of_PFT.keys())[m]] * RADscale_temp_d01 * evi_map_d01[0, m, :, :]) * vegfrac

    # #######################################################
    # # calculate scaled values for d01
    # vegfrac = veg_frac_map_d01[0, m, :, :].values
    # # --- RADscale ---
    # PAR0 = PAR0_of_PFT[list(Tvar_of_PFT.keys())[m]]
    # if PAR0 > 0:
    ...
    #         * SWDOWN_TO_PAR
    #     ) * vegfrac
    #     RADscale_d01 += RADscale_temp_d01
    # # --- Residual --- 
    # Residual_d01 += lambda_of_PFT[list(lambda_of_PFT.keys())[m]] * RADscale_temp_d01 * evi_map_d01[0, m, :, :] * GPP_WRF_d01 * vegfrac
    # # --- Tscale ---
    # a1 = T2_d01 - Tvar_of_PFT[list(Tvar_of_PFT.keys())[m]][1]
    # a2 = T2_d01 - Tvar_of_PFT[list(Tvar_of_PFT.keys())[m]][2]
    # a3 = T2_d01 - Tvar_of_PFT[list(Tvar_of_PFT.keys())[m]][0]
    ...
    #     den = lswi_max_map_d01[0, m, :, :].values - lswi_min_map_d01[0, m, :, :].values
    #     frac = np.where(np.abs(den) < eps, 0, num / den)
    #     Wscale_d01 += frac * vegfrac 
    # else:
    #     Wscale_d01 += (1 + lswi_map_d01[0, m, :, :].values) / (1 + lswi_max_map_d01[0, m, :, :].values) * vegfrac
    # Wscale_d01[np.isnan(Wscale_d01)] = 0 

    # # --- Pscale ---
    # if m == 1:  # evergreen
    #     Pscale_d01 += 1.0 * vegfrac
    # elif m == 4 or m == 6:  # savanna / grassland
    #     Pscale_d01 += (1 + lswi_map_d01[0, m, :, :].values) / 2.0 * vegfrac
    # else:
    #     evithresh = evi_min_map_d01[0, m, :, :].values + 0.55 * (
    #         evi_max_map_d01[0, m, :, :].values - evi_min_map_d01[0, m, :, :].values
    #     )
    ...
    #         (1 + lswi_map_d01[0, m, :, :].values) / 2.0,
    #     ) * vegfrac
    # Pscale_d01[np.isnan(Pscale_d01)] = 0
...
proj_Residual_d01 = proj_on_finer_WRF_grid(
    lats_d01,
    lons_d01,
    Residual_d01,
    lats_fine,
...
all_fields = [RADscale_1km, Tscale_1km, Wscale_1km, Pscale_1km, Residual_1km,GPP_WRF_1km,GPP_test_1km]

for arr in all_fields:
    arr[common_mask] = np.nan

GPP_diff = GPP_test_1km- GPP_WRF_1km
print(np.nanmax(GPP_test_1km))
print(np.nanmax(GPP_test_1km- GPP_WRF_1km))
...
if save_plot:
    ...
    styled_imshow_plot(
        Residual_1km,
        np.nanmin(Residual_1km),
        np.nanmax(Residual_1km),
        "YlOrRd",
        "[-]",
        r"lambdaRadEvi",
    )
...
print_minmax("Residual_1km", Residual_1km)
...
print_minmax("Residual_d01", proj_Residual_d01)
...
Residual_1km       = enforce_positive(Residual_1km)
...
proj_Residual_d01   = enforce_positive(proj_Residual_d01)
...
# Plot normalized LMDI shares
plt.figure(figsize=(8, 4))
plt.bar(shares_norm.keys(), shares_norm.values(), color='tab:orange')
plt.ylabel('Share (%)')
plt.title('Normalized LMDI Contribution Shares (%)')
